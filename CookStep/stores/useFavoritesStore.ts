...
export const useFavoritesStore = create<FavoritesState>((set, get) => ({
  ...
  fetchFavorites: async (userId) => {
    ...
    // 1. Fetch favorite ids for this user
    ...
    if (favError) {
      set({ loading: false, error: favError.message });
      return;
    }
    ...
    if (ids.length === 0) {
      set({ favoriteIds: [], recipes: [], loading: false });
      return;
    }

    // 2. Fetch the full recipe objects
    ...
    if (recipesError) {
      set({ loading: false, error: recipesError.message });
      return;
    }
  ...
  toggle: async (recipeId, userId) => {
    ...
    if (isFav) {
      ...
      // Optimistic add — we need the full recipe to add it to the list
      ...
      if (!error) {
        // Fetch the full recipe to add to the list
        const { data } = await supabase
          ...
          .single();
        if (data) {
          set((state) => ({ recipes: [...state.recipes, data as Recipe] }));
        }
      } else {

...
export function Button({ label, onPress, variant = 'primary', size = 'md', loading = false, disabled = false, style }: ButtonProps) {
  const viewStyles: ViewStyle[] = [
    styles.base, variantViewStyle[variant], sizeStyle[size],
    (disabled || loading) ? styles.disabled : undefined, style,
  ...
  return (
    ...
    >
      {loading
        ...
        : <Text style={[styles.text, variantTextStyle[variant], textSizeStyle[size]]}>{label}</Text>
      }
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  base: { alignItems: 'center', justifyContent: 'center', borderRadius: Spacing.radius.full },
